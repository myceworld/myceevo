...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // myce: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click myce: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start myce: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling myce: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("myce://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'myce://' is not a valid URI. Use 'myce:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // myce: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Myce address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
