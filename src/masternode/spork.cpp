    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        if (mapSporksActive.count(spork.nSporkID)) {
            if (mapSporksActive[spork.nSporkID].nTimeSigned >= spork.nTimeSigned) {
                LogPrintf("%s : seen %s (signed %d)\n", __func__, hash.ToString(), spork.nTimeSigned);
                return;
            } else {
                LogPrintf("%s : got updated spork %s (signed %d)\n", __func__, hash.ToString(), spork.nTimeSigned);
            }
        }

        LogPrintf("%s : new %s ID %d Time %d\n", __func__, hash.ToString(), spork.nSporkID, spork.nValue);
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_14_NEW_PROTOCOL_ENFORCEMENT)
            r = SPORK_14_NEW_PROTOCOL_ENFORCEMENT_DEFAULT;
        if (nSporkID == SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2)
            r = SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2_DEFAULT;
        if (nSporkID == SPORK_16_ZEROCOIN_MAINTENANCE_MODE)
            r = SPORK_16_ZEROCOIN_MAINTENANCE_MODE_DEFAULT;
...
bool CSporkManager::CheckSignature(CSporkMessage& spork, bool fCheckSigner)
{
    ...
    CPubKey pubkey(ParseHex(params.nSporkKey));
    std::string strMessage = boost::lexical_cast<std::string>(spork.nSporkID) + boost::lexical_cast<std::string>(spork.nValue) + boost::lexical_cast<std::string>(spork.nTimeSigned);

    std::string errorMessage;
    return legacySigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage);
}
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_14_NEW_PROTOCOL_ENFORCEMENT")
    ...
    if (strName == "SPORK_16_ZEROCOIN_MAINTENANCE_MODE")
        return SPORK_16_ZEROCOIN_MAINTENANCE_MODE;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_13_ENABLE_SUPERBLOCKS)
        return "SPORK_13_ENABLE_SUPERBLOCKS";
    if (id == SPORK_14_NEW_PROTOCOL_ENFORCEMENT)
        return "SPORK_14_NEW_PROTOCOL_ENFORCEMENT";
    if (id == SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2)
        return "SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2";
    if (id == SPORK_16_ZEROCOIN_MAINTENANCE_MODE)
        return "SPORK_16_ZEROCOIN_MAINTENANCE_MODE";
