...
{
    pathMN = gArgs.GetDataDirNet() / "mncache.dat";
...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    uint256 hash = Hash(ssMasternodes);
    ssMasternodes << hash;
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad, bool fDryRun)
{
    ...
    int fileSize = std::filesystem::file_size(pathMN);
    ...
    uint256 hashTmp = Hash(ssMasternodes);
    if (hashIn != hashTmp) {
...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin, CConnman* connman)
{
    ...
    connman->PushMessage(pnode, msgMaker.Make(NetMsgType::GETMNLIST, vin));
...
{
    int nStable_size = 0;
    int nMinProtocol = PROTOCOL_VERSION - 1;
    ...
    for (CMasternode& mn : vMasternodes) {
        ...
        if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
            nMasternode_Age = TicksSinceEpoch<std::chrono::seconds>(GetAdjustedTime()) - mn.sigTime;
...
void CMasternodeMan::DsegUpdate(CNode* pnode, CConnman* connman)
{
    ...
    const CNetMsgMaker msgMaker(PROTOCOL_VERSION);
    connman->PushMessage(pnode, CNetMsgMaker(pnode->GetCommonVersion()).Make(NetMsgType::GETMNLIST, CTxIn()));
...
//
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(CBlockIndex* pindex, int nBlockHeight, bool fFilterSigTime, int& nCount)
{
    LOCK(cs);
    ...
    */
    ...
    for (CMasternode& mn : vMasternodes) {
        ...
        if (fFilterSigTime && mn.sigTime + (nMnCount * 2.6 * 60) > TicksSinceEpoch<std::chrono::seconds>(GetAdjustedTime()))
            continue;

        // make sure it has as many confirmations as there are masternodes
        // if (mn.GetMasternodeInputAge(nBlockHeight) < nMnCount)
        //    continue;

        vecMasternodeLastPaid.push_back(std::make_pair(mn.SecondsSincePayment(pindex), mn.vin));
    }
    ...
    if (fFilterSigTime && nCount < nMnCount / 3)
        return GetNextMasternodeInQueueForPayment(pindex, nBlockHeight, false, nCount);
...
CMasternode* CMasternodeMan::FindRandomNotInVec(std::vector<CTxIn>& vecToExclude, int protocolVersion)
{
    ...
    int rand = GetRand(nCountEnabled - vecToExclude.size());
...
int CMasternodeMan::GetMasternodeRank(CBlockIndex* pindex, const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
...
std::vector<std::pair<int, CMasternode>> CMasternodeMan::GetMasternodeRanks(CBlockIndex* pindex, int64_t nBlockHeight, int minProtocol)
{
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman* connman)
{
    ...
    if (strCommand == NetMsgType::MNBROADCAST) {
        ...
        if (!legacySigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
        ...
        if (mnb.CheckInputsAndAdd(nDoS, chainman->ActiveChainstate(), connman)) {
            // use this as a peer
            std::vector<CAddress> vecAddress;
            vecAddress.push_back(CAddress(mnb.addr, NODE_NETWORK));
            connman->addrman.Add(vecAddress, pfrom->addr, std::chrono::seconds{2 * 60 * 60});
            masternodeSync.AddedMasternodeList(mnb.GetHash());
        } else {
        ...
        for (CMasternode& mn : vMasternodes) {
            ...
            if (mn.IsEnabled()) {
                ...
                if (vin == CTxIn() || vin == mn.vin) {
                    ...
                    connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::INV, CInv(MSG_MASTERNODE_ANNOUNCE, hash)));
                    nInvCount++;
        ...
        if (vin == CTxIn()) {
            const CNetMsgMaker msgMaker(PROTOCOL_VERSION);
            connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETMNLIST, MASTERNODE_SYNC_LIST, nInvCount));
