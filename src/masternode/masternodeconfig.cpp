...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    std::ifstream streamConfig(pathMasternodeConfigFile);

    if (!streamConfig.good()) {
        FILE* configFile = fopen(pathMasternodeConfigFile.c_str(), "a");
        if (configFile != NULL) {
            std::string strHeader = "# Myce masternode config file\n"
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
            ...
            iss.clear();
            if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                strErr = "Could not parse masternode.conf\n" + strprintf("Line: %d", linenumber) + "\n\"" + std::string(line) + "\"";
                streamConfig.close();
                return false;
            }
        }
